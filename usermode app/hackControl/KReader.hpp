...
public:
	...
	T readType(const int64_t& w_read, const PROTO_MESSAGE& w_protoMsg) const
	{
		...
		{
			readStruct rStruct{ (uint64_t)&writeMe, (uint64_t)w_read, sizeof(T), (uint32_t)GetCurrentProcessId(), 0, TRUE, 0 ,TRUE,0};
			// send the struct to IOCTL
			WriteFile(m_hDriver, (LPCVOID)&rStruct, sizeof(ReadStruct), NULL, NULL);
		}
	...
	{
		int64_t GNames = readType<int64_t>(m_PUBase + 0x36E5710, PROTO_NORMAL_READ);
